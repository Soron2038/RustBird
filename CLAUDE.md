...
End-to-end recipe (verified 2026-09-17, 1.1.14 → 1.1.15): build the `.app` locally _before_ `npm version patch`, install it to `/Applications`, tag and push the release, wait until `latest.json` on `main` shows the new version, relaunch the installed app — it downloads the CI build at launch and "Jetzt neu starten" installs and restarts into it.
